...
def assignKmeans():
    csvdir = './labels/imageInfo_luna.csv'
    centers = np.load('./labels/centers_luna.npy')
    c1 = centers[0]
    c2 = centers[1]
    
    kmeans = np.load('./labels/kmean_luna_raw.npy')
    
    newcsvdir1 = './labels/imagesortInfo_luna1.csv'
    newcsvdir2 = './labels/imagesortInfo_luna2.csv'
...
def processExcludedID():
    abbrv_csv = '../detector/labels/shorter.csv'
    excluded_csv = '../evaluationScript/annotations/annotations_excluded.csv'
    abbrv_exclude_csv = '../detector/labels/excluded.csv'
    
    fullname =[]
    shortname =[]
    
    with open(abbrv_csv, 'r') as f:
        filereader = csv.reader(f)
        for items in filereader:
            fullname.append(items[1])
            shortname.append(items[0])
    ...
    with open(excluded_csv, 'r') as f:
        ...
        for items in filereader:
            excludelist.append(items)
    
    
    for i in range(len(excludelist)):
        items = excludelist[i]
        name = items[0]
        index = fullname.index(name)
        excludelist[i][0] = int(shortname[index])
    
    
    with open(abbrv_exclude_csv, 'w') as file:
        writer = csv.writer(file)
        for items in excludelist:
            writer.writerow(items)
            

if __name__ == '__main__':      
#    lunafull = np.load('./lunafull.npy')
    lunavali = np.load('./luna_test.npy')
    lunavalinp = lunavali.astype(int)
